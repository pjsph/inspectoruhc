...
public class ScoreboardSign {
    ...
    /**
     * Store all the ScoreboardSign instances assigned to a player.
     * @author pjsph
     */
    private static Map<Player, ScoreboardSign> scoreboards = new HashMap<>();

    public static final String TITLE = "§3InspectorUHC";

    private InspectorUHC plugin = InspectorUHC.get();
    ...
    /**
     * Create a scoreboard sign for a given player using the default title.
     * @author pjsph
     * @param player
     */
    ...
    /**
     * Update all the lines of the main scoreboard (preset lines).
     * @author pjsph
     */
    public void update() {
        this.setLine(0, "");
    ...
    private String getFormattedTimer(int minutes, int seconds) {
        String minS = String.valueOf(minutes);
        String secS = String.valueOf(seconds);

        if(minutes <= 9) minS = "0" + minS;
        if(seconds <= 9) secS = "0" + secS;

        return minS + ":" + secS;
    }

    private String getFormattedEpisode(int episode) {
        String epS = String.valueOf(episode);

        if(episode <= 9) epS = "0" + epS;
    ...
    public static Map<Player, ScoreboardSign> getScoreboards() {
        return scoreboards;
    }
}